...
Do not mention details about your training, limitations or weaknesses.
Start by writing bullet points of the acceptance criteria for this task.
```

**2) Write the tests**

```mdx
Your next task is to write a suite of self contained tests.
that will test the code that you are about to write.
Ensure the the tests are tied to the acceptance criteria we defined.
...
```mdx
Your next task is to write the code that will pass the tests.
Ensure that the code is true to the acceptance criteria.
Remember to abide by the coding principals that you value.
Begin:
```
